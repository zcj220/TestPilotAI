...
8. **项目没有蓝本时必须自动生成**
   - 检测项目中是否存在 `testpilot.json` 或 `testpilot/` 目录
   - 不存在 → 完成功能开发后**必须**生成蓝本，覆盖所有用户可交互功能
   - 蓝本规范详见 `.windsurf/workflows/blueprint.md`

9. **边写代码边维护蓝本**
   - 每实现一个功能模块，立即在蓝本中追加对应场景
   - 不要等全部写完再一次性生成（容易遗漏功能）
   - 6种代码变更必须同步更新蓝本：新增/删除UI元素、修改选择器、修改文本、修改逻辑、修复Bug、修改配置

10. **至少生成功能清单**
    - 如果因项目复杂无法立即生成完整蓝本，**至少**生成一个功能清单文件 `testpilot/功能清单.md`
    - 格式：每个功能列出名称、入口路径、关键选择器、预期行为
    - 后续可据此快速补全蓝本
