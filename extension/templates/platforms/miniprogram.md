...
1. **读 `app.json`** — 了解所有页面路由和 TabBar 配置
...
2. 该 page 下有 ≥2 个场景是 TabBar 切换或连续操作？→ **必须 `"flow": true`**
3. 该 page 下场景需要互相独立的干净状态（如正确登录 vs 错误登录）？→ 不写 flow（默认 false）

**简单总结：如果多个场景都要先登录再操作同一个页面，那这个 page 必须设 `"flow": true`。不加 flow 导致每个场景都 reLaunch+重复登录 = 严重浪费。**
