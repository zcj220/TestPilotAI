...
### 🚨 绝对禁止：用 React 组件名猜测 Lucide/图标 SVG 的 class（大小写陷阱）

**React 图标库（Lucide React、Heroicons、react-icons 等）渲染后的 SVG class 与组件名 `PascalCase` 完全不同！** AI 常看到 `<ArrowLeft />` 就写 `svg[class*='ArrowLeft']`，但 Lucide 渲染后实际 class 是全小写 kebab-case。

| React 写法 | ❌ 错误选择器（必然超时） | ✅ 正确选择器 |
|---|---|---|
| `<ArrowLeft />` | `svg[class*='ArrowLeft']` | `svg.lucide-arrow-left` 或 `button.p-2 svg` |
| `<Plus />` | `svg[class*='Plus']` | `svg.lucide-plus` 或父按钮的 class |
| `<Search />` | `svg[class*='Search']` | `svg.lucide-search` |
| `<Trash2 />` | `svg[class*='Trash']` | `svg.lucide-trash-2` |

**Lucide class 命名规则**：组件名 `PascalCase` → 全小写 kebab-case 加前缀 `lucide-`。`ArrowLeft` → `lucide-arrow-left`，`Trash2` → `lucide-trash-2`。

**更好的做法**：不要定位 svg 本身，而是**定位包裹 svg 的按钮**。如果按钮有 `title` 属性（如 `title="返回"`），用 `button[title='返回']`；如果没有 title，用按钮自身的 class。

> ⚠️ 读源码时还需确认：`title="xxx"` 只是 tooltip 属性，**不是页面可见文本**，不能用 `assert_text` 断言。应改用 `assert_visible: button[title='xxx']`。
