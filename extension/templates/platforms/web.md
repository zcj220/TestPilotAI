...
```json
{
  ...
  "flow": true,
  "scenarios": [
    {"name": "查看统计图", "steps": [{"action": "navigate", "value": "/dashboard"}, ...]},
    {"name": "切换时间范围", "steps": [{"action": "navigate", "value": "/dashboard"}, ...]},
    {"name": "导出报表", "steps": [{"action": "navigate", "value": "/dashboard"}, ...]}
  ]
}
```
...
- 步骤失败时AI中枢决策恢复，尽量跳过继续

**适用场景：** 同页面 Tab 切换、连续 CRUD 操作、页面间导航测试。
**重要：** 每个场景仍需写 navigate（方便单独运行），flow 模式下引擎自动跳过。
