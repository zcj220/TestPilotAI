...
两种模式：手机浏览器测试（CSS选择器）和原生App测试。

### 手机浏览器模式（H5）
选择器与Web相同（CSS），base_url用局域网IP（不能用localhost）。

### 原生App / Flutter App 模式
...
1. accessibility_id:xxx — 按无障碍ID查找（按钮/文本/图标首选）
2. //android.widget.EditText[@hint='xxx'] — 按hint查找输入框（Flutter TextField必用）
3. //ClassName[@attribute='value'] — 其他XPath精确定位
4. resource-id:xxx — 按resource-id查找（非Flutter原生View）

❌ 绝对禁止：UiSelector().instance(N)、//xxx[@index='N']、div:nth-child(N)、纯ClassName

#### Flutter Semantics → Android 无障碍属性映射（核心）

| Flutter 代码 | Android属性 | 蓝本选择器 |
|---|---|---|
...
| TextField(hintText:'xxx')（无Semantics包裹） | hint="xxx" | //android.widget.EditText[@hint='xxx'] |

⚠️ 优先级：Semantics label 覆盖 TextField 的 hintText。
   如 Semantics(label:'tf_user') 包裹 TextField(hintText:'请输入')，hint='tf_user'。

#### wait 动作两种格式

- 简单等待：{"action": "wait", "value": "3000"} — 固定等待3秒
- 等待元素：{"action": "wait", "target": "accessibility_id:xxx", "timeout_ms": 15000}

Flutter 每次 navigate 后必须：wait 3000 + wait target元素(15s)

#### navigate 重启应用

{"action": "navigate", "value": "com.包名/.Activity"}
引擎会 force-stop → 重建Session → 重启app。用于场景间重置。
...
|---|---|
| App冷启动 | 3000 |
| 场景间navigate重启 | 3000 + wait target关键元素 |
| click后等跳转 | 2000 |
| 普通操作后 | 1000 |

支持的 action：navigate / click / fill / select / wait / screenshot / assert_text / assert_visible / scroll
Bug标记：description中加"【预期失败-Bug-N】原因说明"
每个click必须有assert_text或screenshot验证。"""
