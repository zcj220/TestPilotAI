...
- [x] **v1.2** — 原生Android验证（95%通过率 | Y键/S键Bug修复 | assert文本修复 | xpath前缀 | 输入法每次检查）
- [x] **v1.2.1** — 蓝本提示词优化（绝对正向验证 + 代码是唯一真相 + 穷举输入变体 + 蓝本备忘录CHANGELOG.md）
- [ ] **v1.3** — Windows桌面测试验证 + macOS支持
- [ ] **v1.4** — 社区经验库（用户授权上传测试经验，众包测试知识共享）
- [ ] **v1.5** — 自动闭环增强（插件一键启动引擎 + 自动测试→修复→重测循环）
- [ ] **v1.6** — 输入容错机制（输入后验证 + 自动重试 + 多种输入方案降级）
