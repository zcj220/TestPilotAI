...
**每次修改代码后，你必须主动审视并更新对应的蓝本，无需用户提醒。**

### 自动更新触发条件

当你完成以下任何代码修改后，**立即检查并更新对应模块的蓝本**：

1. **新增UI元素**：新增按钮/表单/导航/弹窗 → 更新对应模块蓝本，添加新场景
2. **修改元素ID/class**：修改了 `id` 或 `class` → 更新蓝本中的选择器
3. **修改业务逻辑**：修改表单验证/计算逻辑/跳转流程 → 更新蓝本中的断言和预期结果
4. **删除功能**：删除某个功能 → 从蓝本中删除对应场景
5. **修复Bug**：修复了Bug → 更新蓝本中的 `expected` 断言，确保能检测到该Bug
6. **修改URL/端口**：修改了应用端口或路由 → 更新蓝本中的 `base_url` 和 `start_command`

### 自动审视流程

```
修改代码 → 自问：这个改动影响哪个功能模块？
         → 找到对应的 xxx.testpilot.json
         → 检查：选择器是否还正确？断言是否需要更新？
         → 更新蓝本
         → 完成
```
