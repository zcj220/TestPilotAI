...
## 代码变化时自动审视并更新蓝本（智能检查，避免token浪费）

**只在关键变更时检查蓝本，避免不必要的token消耗。**

### ✅ 必须检查并更新蓝本的情况

当你完成以下**关键变更**后，**立即检查并更新对应模块的蓝本**：

1. **新增/删除UI元素**：新增或删除按钮/表单/导航/弹窗 → 更新对应模块蓝本，添加/删除场景
2. **修改元素ID/class**：修改了 `id` 或 `class` → 更新蓝本中的选择器
3. **修改业务逻辑**：修改表单验证/计算逻辑/跳转流程 → 更新蓝本中的断言和预期结果
4. **修复Bug**：修复了Bug → 更新蓝本中的 `expected` 断言，确保能检测到该Bug
5. **修改URL/路由/端口**：修改了应用端口或路由 → 更新蓝本中的 `base_url` 和 `start_command`
6. **修改文本内容**：修改了按钮文字、提示信息等 → 更新蓝本中的 `assert_text` 断言

### ❌ 不需要检查蓝本的情况（节省token）

以下变更**不影响测试**，无需检查蓝本：

1. **修改CSS样式**：颜色/字体/布局/动画 → 跳过
2. **修改注释/文档**：代码注释/README → 跳过
3. **重构代码（功能不变）**：提取函数/重命名变量（但元素ID/class不变）→ 跳过
4. **修改后端逻辑（前端不变）**：数据库查询优化/API性能优化 → 跳过
5. **性能优化**：缓存/懒加载/代码分割 → 跳过
6. **添加日志/调试代码** → 跳过

### 智能判断流程（快速决策，节省token）

```
修改代码 → 快速判断：是否修改了HTML/JSX中的 id/class/文本/结构？
...
| 修改内容 | 是否检查蓝本 | 耗时 |
|---------|-------------|------|
| HTML中的 `id="xxx"` | ✅ 必须 | 10秒 |
| HTML中的 `class="xxx"` | ✅ 必须 | 10秒 |
| 按钮文字/提示信息 | ✅ 必须 | 10秒 |
| 表单验证逻辑 | ✅ 必须 | 20秒 |
| CSS样式 | ❌ 跳过 | 0秒 |
| 注释/文档 | ❌ 跳过 | 0秒 |
